...
{
    public class ScheduleTables1Controller : Controller
    {
        ...
        {
            var scheduleTables = db.ScheduleTables.Include(s => s.Day).Include(s => s.Group).Include(s => s.Para).Include(s => s.Room).Include(s => s.Subject.UPlanId).Include(s => s.Teacher);
            return View(scheduleTables.ToList());
        }

        // GET: ScheduleTables1/Details/5
